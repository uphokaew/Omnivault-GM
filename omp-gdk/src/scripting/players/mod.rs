...
impl Player {
    ...
    pub fn send_client_message(&self, colour: Colour, message: &str) {
		println!("send_client_message: {:?}", message.as_bytes());
        functions::SendClientMessage(self, colour, message);
    }

	// pub fn send_client_message(&self, colour: Colour, message: &str) {
   	// 	 // แปลงจาก UTF-8 → TIS-620 (Windows-874)
	// 	match WINDOWS_874.encode(message, EncoderTrap::Replace) {
	// 		Ok(ansi_bytes) => {
	// 			println!("Encoded ANSI (TIS-620) bytes: {:?}", ansi_bytes);

	// 			// แปลงเป็น CString (ปลอดภัยสำหรับ FFI หรือ C-based APIs)
	// 			if let Ok(c_string) = CString::new(ansi_bytes) {
	// 				// ส่งข้อความที่แปลงแล้วไปยังฟังก์ชัน SendClientMessage
	// 				functions::SendClientMessage(self, colour, c_string.to_str().unwrap_or("????????????????????"));
	// 			} else {
	// 				eprintln!("แปลงเป็น CString ไม่สำเร็จ (อาจมี null byte)");
	// 			}
	// 		},
	// 		Err(e) => {
	// 			eprintln!("เกิดข้อผิดพลาดระหว่างแปลง encoding: {:?}", e);
	// 		}
	// 	}
	// }
